...
def atualizar_store_limpar_campos_form_adv(n_save, n_cancel, n_open, n_intervals, dataset, nome, oab, cpf, is_open):
    ...
    if not ctx.triggered:
        ...
        return dataset, [], {}, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update # False
    ...
    if trigger_id == 'save_button_novo_advogado':
        if None in [nome, oab, cpf]:
            ...
            return dash.no_update, ['Todos os dados são obrigatórios para registro!'], \
                   ...
                   nome, oab, cpf, True, dash.no_update #, False
        ...
        if str(oab) in df_adv['OAB'].values:
            return dash.no_update, ['Número de OAB já existe no sistema!'], {'margin-bottom': '15px', 'color': 'red'}, nome, oab, cpf, True, False #, False
        
        elif str(cpf) in df_adv['CPF'].values:
            return dash.no_update, ['Número de CPF já existe no sistema!'], {'margin-bottom': '15px', 'color': 'red'}, nome, oab, cpf, True, False #, False
        
        elif not validar_cpf(cpf):
            return dash.no_update, ['Número de CPF inválido!'], {'margin-bottom': '15px', 'color': 'red'}, nome, oab, cpf, True, False #, False

        elif not validar_oab(oab):
            return dash.no_update, ['Número OAB inválido!'], {'margin-bottom': '15px', 'color': 'red'}, nome, oab, cpf, True, False #, False
        
        elif nome in df_adv['Advogado'].values:
            return dash.no_update, [f'Nome {nome} já existe no sistema!'], {'margin-bottom': '15px', 'color': 'red'}, nome, oab, cpf, True, False #, False
        ...
        return dataset, ['Cadastro realizado com sucesso!'], \
               {'margin-bottom': '15px', 'color': 'green'}, '', '', '', True, False #, True


    # Fallback padrão
    return dash.no_update, [], {}, nome, oab, cpf, is_open, dash.no_update #, False
